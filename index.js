...
const PORT = process.env.PORT || 3000;ocess.env.CLIENT_SECRET;
const REDIRECT_
const CLIENT_ID = process.env.CLIENT_ID;
const CLIENT_SECRET = prURI = process.env.REDIRECT_URI || `https://industrious-creation-production-dddb.up.railway.app/api/auth/callback`;
...
const guildConfigSchema = new mongoose.Schema({
              ...
              recruitment: {
                                ...
                                title: { type: String, default: 'Application Submission' },
...
client.on('interactionCreate', async interaction => {
              if (interaction.isButton()) {
                                ...
                                if (!config || !config.recruitment || !config.recruitment.open) {
                                                      return interaction.reply({ content: 'Recruitment is currently closed.', ephemeral: true });
                                }

                  const modal = new ModalBuilder()
                                    .setCustomId('apply_modal')
                                    .setTitle(config.recruitment.title || 'Application Submission');

                  modal.addComponents(
                                        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('q1').setLabel('What is your name?').setStyle(TextInputStyle.Short).setRequired(true)),
                                        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('q2').setLabel('How old are you?').setStyle(TextInputStyle.Short).setRequired(true)),
                  ...
                  if (config && config.adminChannelId) {
                                        ...
                                        if (channel) {
                                                                  const embed = new EmbedBuilder()
                                                                      ...
                                                                      .addFields(
                                                                                { name: 'Name', value: interaction.fields.getTextInputValue('q1') || 'No response' },
                                                                                { name: 'Age', value: interaction.fields.getTextInputValue('q2') || 'No response' },
                                                                                { name: 'Reason', value: interaction.fields.getTextInputValue('q3') || 'No response' }
                                                                                                    )
